...
import time
import pickle
from numpy import array, zeros, r_, savetxt
...
def load_images_to_array(classification_label_and_values):
    ...
    for class_label, class_value in classification_label_and_values.iteritems():#iteritems 获取字典的键值
        for filename in glob.glob("./"+class_label+"/*"):#获取当前文件下class_label的路径
            image_array = imread(filename, flatten=True)
            resized_image_array = imresize(image_array, IMAGE_DIMENSIONS)  #修改训练集照片的尺寸大小
            training_image_array = r_[training_image_array, [resized_image_array.flatten()]]# 将一系列的序列合并到一个数组中
...
def main():
    ...
    x_values = image_array[1:, :] #输入数据集
    y_values = image_values[1:, :] # 输出数据集
    #savetxt('model/input.csv', x_values, delimiter = ',')
    #savetxt('model/output.csv', y_values, delimiter = ',')
